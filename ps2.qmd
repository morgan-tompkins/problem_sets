...
d %>%
  group_by(DoNotUse) %>%
  summarize(n = n()) # there are three entries marked as DoNotUse. Which ones?

d %>%
  filter(DoNotUse == 1) %>%
  select(Subject, DoNotUse)
  

filtered_d <- d %>% 
  filter(is.na(DoNotUse)) # your code here: exclude subjects that are marked as "DoNotUse"

nrow(d) - nrow(filtered_d) # check
```
...
```{r add-errobars}
# Add SEM
perforamance_dff_SEM_d <- performance_diff_d %>%
  group_by(ConfrontationalGame, MusicCondition) %>%
  summarise(
    MeanImprovementScore = mean(ImprovementScore, na.rm = TRUE),
    SEM = sd(ImprovementScore, na.rm = TRUE) / sqrt(sum(!is.na(ImprovementScore)))
  )

# Plot SEM
ggplot(perforamance_dff_SEM_d, 
